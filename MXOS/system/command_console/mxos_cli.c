...
static const mxos_uart_config_t cli_uart_config =
{
...
static void get_version(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
  ...
  int ret;
  mxos_sdk_version( &major, &minor, &revision );
...
{
  if (argc == 1) {
    cmd_printf("Usage: mxosdebug on/off. MXOS debug is currently %s\r\n",
               mxos_debug_enabled ? "Enabled" : "Disabled");
    return;
  }
  
  if (!strcasecmp(argv[1], "on")) {
    cmd_printf("Enable MXOS debug\r\n");
    mxos_debug_enabled = 1;
  } else if (!strcasecmp(argv[1], "off")) {
    cmd_printf("Disable MXOS debug\r\n");
    mxos_debug_enabled = 0;
  }
}

static const struct cli_command user_clis[1] = {
  {"mxosdebug", "mxosdebug on/off", mxosdebug_Command},
};
#endif
...
int cli_init(void)
{
  ...
  MxosUartInitialize( MXOS_CLI_UART, &cli_uart_config, (ring_buffer_t*)&cli_rx_buffer );
...
{
  if (msg[0] != 0)
    MxosUartSend( MXOS_CLI_UART, (const char*)msg, strlen(msg) );
...
int cli_getchar(char *inbuf)
{
  if (MxosUartRecv(MXOS_CLI_UART, inbuf, 1, MXOS_WAIT_FOREVER) == 0)
    return 1;
  else
    return 0;
}
