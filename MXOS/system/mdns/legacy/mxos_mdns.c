...
/* Debug functions */
#define mdns_utils_log(M, ...)  MXOS_LOG(MXOS_MDNS_DEBUG, "MDNS", M, ##__VA_ARGS__)
#define mdns_utils_log_trace()  MXOS_LOG_TRACE("MDNS")
...
static uint32_t dns_write_record_AAAA( dns_message_iterator_t* iter, dns_sd_service_record_t *service, uint16_t record_class )
{
    ...
    memset(ipv6_addrs, 0x0, MXOS_IPV6_NUM_ADDRESSES*sizeof(ipv6_addr_t));
    mxosWlanGetIP6Status(ipv6_addrs, MXOS_IPV6_NUM_ADDRESSES, service->interface);

    for( int i = 0; i < MXOS_IPV6_NUM_ADDRESSES && IP6_ADDR_IS_VALID(ipv6_addrs[i].addr_state); i++ ) {
...
static merr_t start_bonjour_service(void)
{
  ...
  err = mxos_system_notify_register( mxos_notify_WIFI_STATUS_CHANGED, (void *)BonjourNotify_WifiStatusHandler, NULL );
  require_noerr( err, exit );
