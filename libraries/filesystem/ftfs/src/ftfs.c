...
size_t ft_fread( void *ptr, size_t size, size_t nmemb, file *f )
{
    ...
    for ( i = 0; i < nmemb; i++ )
    {
        ...
        mhal_flash_read( MXOS_PARTITION_FILESYS, &addr, (uint8_t *) (b + len), size );
        len += size;
        stream->fp += size;
        if ( stream->fp >= stream->length )
            break;
...
struct fs *ftfs_init( struct ftfs_super *sb, mxos_partition_t partition )
{
    FT_HEADER sec;
...
bool ft_is_content_valid( mxos_partition_t partition, int be_ver )
{
    FT_HEADER sec1;
    bool ret = false;

    if ( ft_read_header( &sec1, mxos_partitions[partition].partition_start_addr ) != kNoErr )
        goto ret;
