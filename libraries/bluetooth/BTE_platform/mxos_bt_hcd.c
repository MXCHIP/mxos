...
void get_one_command(char * out, int offset)
{
    ...
    mxos_logic_partition_t *driver_partition = mhal_flash_get_info( MXOS_PARTITION_BT_FIRMWARE );

    if( image_size == 0)
      image_size = driver_partition->partition_length;

    mhal_flash_read( MXOS_PARTITION_BT_FIRMWARE, &read_address, (uint8_t *)&len, 1);
    len = len&0x00ff;
    read_address = offset;
    mhal_flash_read( MXOS_PARTITION_BT_FIRMWARE, &read_address, (uint8_t*)out, len+3);
...
#define READ_LEN 2048
    if(image_size==0)
    {
        ...
        do {
            ...
            mhal_flash_read( MXOS_PARTITION_BT_FIRMWARE, &offset, (uint8_t *)buf, READ_LEN);
