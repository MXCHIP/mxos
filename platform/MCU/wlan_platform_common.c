...
#define READ_LEN 2048
    mxos_logic_partition_t *driver_partition = mhal_flash_get_info( MXOS_PARTITION_RF_FIRMWARE );
    ...
    do {
        ...
        mhal_flash_read( MXOS_PARTITION_RF_FIRMWARE, &offset, (uint8_t *)buf, READ_LEN);
...
uint32_t platform_get_wifi_image(unsigned char* buffer, uint32_t size, uint32_t offset)
{
    ...
    mhal_flash_read( MXOS_PARTITION_RF_FIRMWARE, &read_address, buffer, buffer_size);
    return buffer_size;
}
#endif
