...
mret_t mico_wlan_monitor_set_channel(uint8_t channel);
...
mret_t mico_platform_init(void)
{
	return mxos_platform_init();
}

mret_t mico_rtos_deinit_queue(mxos_queue_t *queue)
{
...
mret_t mico_rtos_init_queue(mxos_queue_t *queue, const char *name, uint32_t message_size, uint32_t number_of_messages)
{
...
mret_t mico_rtos_pop_from_queue(mxos_queue_t *queue, void *message, uint32_t timeout_ms)
{
	return mxos_rtos_pop_from_queue(queue, message, timeout_ms);
}

mret_t mico_rtos_thread_force_awake(mos_thread_id_t *thread)
{
